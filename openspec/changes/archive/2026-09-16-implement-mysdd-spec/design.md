...
共通手順をそこから取得している。DOCX変換自体はトップレベルの
`scripts/openspec/`に実装済みだが、Agent Skill固有の実行資産としては
Skill Directory外にあり、Package単体で移植できない。

一方、MySDD仕様は共通変換をAgent Skill `md2docx`として分離すること、
...
- `md2docx`をDOCX変換契約の唯一のAgent Skillにする。
- 文書内容の生成と形式変換の責務を分離する。
- 既存RendererをSkillの`scripts/`へ移し、参照DOCX、出力先、部分成功の
  扱いを維持する。
...
### `md2docx`を薄いAgent Skillとして追加する

`.agents/skills/md2docx/`に`SKILL.md`と`scripts/`を置く。`SKILL.md`は
変換前の入力検証、許可する出力先、Rendererの呼び出し、成功判定、失敗報告
だけを定義する。文書内容の生成、章立て、`TBD`補完、トレーサビリティ作成は
担当しない。
...
pathとしてSkill内の`scripts/render-docx.ps1`へ渡す。変換テストも
`scripts/test-render-docx.ps1`として同じSkill Packageに置く。
...
`md2docx`の手順へ委譲する。必須Artifact不足は各生成Skill、変換入力不足と
Renderer失敗は`md2docx`が報告する。DOCX変換失敗時も生成済みMarkdownを
保持し、生成全体を部分成功として扱う。

移行は`md2docx`追加、2 Skillの参照更新、参照切れ検査、旧`_shared`削除の
...
`Implemented`へ変更する。想定Directory構成は`md2docx`を実在する構成として
...
| `QR-007` | 変換規則を`md2docx/SKILL.md`へ一元化する | Skill間の委譲が1段増える | 旧`_shared`参照0件の検索結果 |
...
- `[出力先制約が既存Scriptより厳しい]` → 制約はSkill境界で検証し、汎用の
  RendererはSkill内へ移し、出力先制約は`SKILL.md`で検証する。
- `[文書だけがImplementedになり実装が未完了になる]` → Status変更を全検証の
  最終Taskにし、失敗時は`Proposed`を維持する。

## Migration Plan

1. `md2docx/SKILL.md`を追加し、RendererとテストをSkillの`scripts/`へ移す。
